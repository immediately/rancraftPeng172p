...
public class EntityPenguinFishHook extends EntityFishHook
{
	int clickDuration = 0; // just trying to distinguish it better from EntityFishHook
    ...
    //public EntityPlayer field_146042_b; /* We DON'T want a new player at this level */
    ...
    public EntityPenguinFishHook(World par1World)
    {
        super(par1World);
...
/*    @SideOnly(Side.CLIENT)
    public FH(World par1World, double par2, double par4, double par6, EntityPlayer par8EntityPlayer)
    {
        this(par1World);
...
*/
    public EntityPenguinFishHook(World par1World, EntityPlayer par2EntityPlayer)
    {
        super(par1World);
        this.field_146037_g = -1;
        this.field_146048_h = -1;
...
*/
    ...
    public void onUpdate()
    {
		...
		//System.out.printf("EntityPenguinFishHook: Updating.\n");
        ...
        else
        {
            if (!this.worldObj.isRemote)
            {
    			...
    			if (this.field_146042_b.isDead || !this.field_146042_b.isEntityAlive() || itemstack == null || itemstack.getItem() != RanCraftPenguins.PenguinFishingRod || this.getDistanceSqToEntity(this.field_146042_b) > 1024.0D)
                {
                    Item possibleFishingRod = (itemstack != null)?itemstack.getItem():null;
                    if(possibleFishingRod != null && possibleFishingRod == RanCraftPenguins.PenguinFishingRod){
            			//System.out.printf("Calling set rod icon to empty in ItemPenguinFishingRod.\n");
                    	((ItemPenguinFishingRod) possibleFishingRod).setIconTo(0);
                    }
                    this.setDead();
            ...
            if (!this.field_146051_au)
            {
                ...
                if (!this.worldObj.isRemote && d10 > 0.0D)
                {
                    ...
                    else
                    {
                        ...
                        {
                            //this.field_146040_ay = MathHelper.getRandomIntegerInRange(this.rand, 100, 900); /* original */
                            //this.field_146040_ay = MathHelper.getRandomIntegerInRange(this.rand, 10, 50); /* very fast for testing */
                            this.field_146040_ay = MathHelper.getRandomIntegerInRange(this.rand, 72, 648); /* 40% faster */
    ...
    public int func_146034_e()
    {
    	...
    	numFish = MathHelper.getRandomIntegerInRange(this.rand, 1, 3);
    ...
    private ItemStack func_146033_f()
    {
        ...
        //float f = 0.0F; // all bad stuff
        //float f = 0.1F; // all good stuff
        //float f = 0.2F; // all fish
        f = MathHelper.clamp_float(f, 0.0F, 0.9F);
